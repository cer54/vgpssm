...
for e = 1:E
  ...
  for n = 1:size(qx,2)
    ...
    r1 = prod(diag(chol(eye(F)+iL*Sd*iL)));                        % sqrt det
    %  s = bsxfun(@minus, z(:,:,e), [qx(n).m(:,t)'
    %                          u{n}(t,:)]); 
    s = bsxfun(@minus, z(:,:,e), [qx(n).m(:,t)' u{n}(t,:)]); 
    a = s/(L2+Sd);
    b3(e,1) = w'*(exp(-sum(a.*s,2)/2)/r1); b4 = zeros(M,E);
    for f=1:E, b4(:,f) = a(:,f).*exp(-sum(a.*s,2)/2)/r1;end
    b5(e, :) = w'*b4;
  end
end
qx_opt(n).m = b3;
qx_opt(n).So = qx.Sd*b5;
qx_opt(n).Sd = exp(2*hyp(e).pn) + qx_opt(n).So * inv(qx(n).Sd(:, :, end)) ...
    * qx_opt(n).So';
